...
export const ensureAdminAuth = async (): Promise<boolean> => {
  try {
    ...
    console.log('🔐 [AdminAuth] No Supabase session found, checking admin localStorage auth...');
    ...
    if (isAdminAuthenticated) {
      console.log('🔑 [AdminAuth] Admin authenticated via localStorage, creating anonymous session...');
      ...
      try {
        ...
        if (error) {
          console.warn('⚠️ [AdminAuth] Anonymous session creation failed:', error.message);

          // Check if anonymous auth is disabled - this is common in production
          if (error.message.includes('Anonymous sign-ins are disabled')) {
            console.log('🔓 [AdminAuth] Anonymous auth disabled, but admin is authenticated - allowing operations');
            return true; // Continue with admin privileges
          }

          // For other errors, still allow admin operations
          console.log('🔓 [AdminAuth] Session creation failed but admin is authenticated - allowing operations');
          return true;
        }
        ...
        console.warn('⚠️ [AdminAuth] Session creation error:', sessionError);
        console.log('🔓 [AdminAuth] Admin is authenticated - allowing operations despite session error');
        return true;
      }
    }

    console.log('🔐 [AdminAuth] No admin authentication found, attempting anonymous session...');

    // Try to create anonymous session for public operations
    const { data, error } = await supabase.auth.signInAnonymously();

    if (error) {
      console.warn('⚠️ [AdminAuth] Anonymous session creation failed:', error.message);
      console.log('🔓 [AdminAuth] Continuing with public access - relying on RLS policies');
      return true; // Allow operations to continue with public policies
    }
    ...
    console.log('🔓 [AdminAuth] No session created but continuing with public access');
    return true;
  } catch (error) {
    console.error('❌ [AdminAuth] Error ensuring admin authentication:', error);
    console.log('🔓 [AdminAuth] Falling back to public access with RLS policies');
