...
const OrderNotificationSystem = () => {
  ...
  useEffect(() => {
    ...
    const initializeAudio = () => {
      try {
        // Initialize pleasant notification sound system
        console.log('🎵 [OrderNotification] Initializing pleasant notification sounds...');

        // Test the pleasant notification sound system
        const soundStatus = pleasantNotificationSound.getStatus();
        console.log('🎵 [OrderNotification] Pleasant sound status:', soundStatus);

        // Create a simple fallback audio element for compatibility
        const audio = new Audio();
        audio.loop = true;
        audio.volume = 0.8;

        // Use the new pleasant notification-sound.wav as fallback
        ...
        audio.onerror = () => {
          console.log('🎵 [OrderNotification] MP3 file not found, will use pleasant generated sounds');

          // Don't create harsh beep fallback - we'll use pleasant sounds instead
    ...
    return () => {
      try {
        ...
        if ((window as any).pleasantSoundActive) {
          pleasantNotificationSound.stopContinuousRinging();
          (window as any).pleasantSoundActive = false;
        }

        // Clean up old pleasant sound loop (for backward compatibility)
        if ((window as any).pleasantSoundLoop) {
          clearInterval((window as any).pleasantSoundLoop);
          (window as any).pleasantSoundLoop = null;
        }
  ...
  // Start continuous notification sound - now using pleasant sounds!
  const startNotificationSound = async () => {
    try {
      console.log('🎵 [OrderNotification] ===== STARTING PLEASANT SOUND =====');
      ...
      console.log('🔔 [OrderNotification] Starting truly continuous ringing notification...');
      const ringingSuccess = await pleasantNotificationSound.startContinuousRinging();

      if (ringingSuccess) {
        console.log('🔔 [OrderNotification] ✅ Continuous ringing started successfully - will ring until stopped');
        setIsPlaying(true);

        // No need for intervals - the ringing is truly continuous until manually stopped
        // Store reference for cleanup (no interval needed)
        (window as any).pleasantSoundActive = true;
        return;
      } else {
        console.log('🔔 [OrderNotification] Ringing sound failed, trying fallback...');
      }
  ...
  const forceStopSound = useCallback(() => {
    ...
    try {
      // 0. Stop continuous ringing first
      if ((window as any).pleasantSoundActive) {
        console.log('🔇 [OrderNotification] 🔔 STOPPING CONTINUOUS RINGING');
        pleasantNotificationSound.stopContinuousRinging();
        (window as any).pleasantSoundActive = false;
        console.log('🔇 [OrderNotification] ✅ Continuous ringing stopped');
      }

      // Also clear any old interval-based loops (for backward compatibility)
      if ((window as any).pleasantSoundLoop) {
        console.log('🔇 [OrderNotification] 🎵 STOPPING OLD SOUND LOOP');
        clearInterval((window as any).pleasantSoundLoop);
        (window as any).pleasantSoundLoop = null;
        console.log('🔇 [OrderNotification] ✅ Old sound loop stopped');
      }

      // 1. Stop iOS audio loop
      if ((window as any).stopIOSLoop) {
        console.log('🔇 [OrderNotification] 🍎 STOPPING iOS AUDIO LOOP');
        (window as any).stopIOSLoop();
        (window as any).stopIOSLoop = null;
