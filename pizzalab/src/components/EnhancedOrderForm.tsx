...
const EnhancedOrderForm = () => {
  ...
  const createOrder = async () => {
    ...
    // Create standardized notification with detailed logging
    console.log('🔔 [EnhancedOrderForm] STARTING notification creation for order:', order.id);
    console.log('🔔 [EnhancedOrderForm] Order details:', {
      orderId: order.id,
    ...
    try {
      console.log('🔔 [EnhancedOrderForm] Attempting notification insert...');
      ...
      if (notificationError) {
        console.error('❌ [EnhancedOrderForm] NOTIFICATION CREATION FAILED:', notificationError);
        console.error('❌ [EnhancedOrderForm] Error details:', {
        ...
        console.log('🔄 [EnhancedOrderForm] Trying database function fallback...');
        const { data: functionResult, error: functionError } = await supabase
          .rpc('create_order_notification', {
            p_order_id: order.id,
            p_notification_type: 'new_order',
            p_message: `Nuovo Ordine! New order from ${formData.customerName}`
          });

        if (functionError) {
          console.error('❌ [EnhancedOrderForm] DATABASE FUNCTION ALSO FAILED:', functionError);
        } else {
          console.log('✅ [EnhancedOrderForm] Database function succeeded:', functionResult);
        }
      } else {
        console.log('✅ [EnhancedOrderForm] NOTIFICATION CREATED SUCCESSFULLY:', notificationData);

        // Dispatch custom event to trigger notification system
        console.log('📡 [EnhancedOrderForm] Dispatching newOrderReceived event');
        ...
        console.log('🚨 [EnhancedOrderForm] Force triggering notification system');
        setTimeout(() => {
          window.dispatchEvent(new CustomEvent('forceNotificationCheck'));
        }, 500);
      }
    } catch (exception) {
      console.error('❌ [EnhancedOrderForm] NOTIFICATION CREATION EXCEPTION:', exception);
      console.error('❌ [EnhancedOrderForm] Exception details:', {
        name: exception.name,
