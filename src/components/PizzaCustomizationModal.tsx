...
const PizzaCustomizationModal: React.FC<PizzaCustomizationModalProps> = ({
  ...
  onAddToCart,
  initialQuantity = 1,
  categorySettings = { impasto_enabled: true, aggiunti_enabled: true, bevande_enabled: true }
}) => {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      if (categorySettings.bevande_enabled) {
        loadBevande();
      }
      ...
      // Set initial step based on what's enabled
      if (categorySettings.impasto_enabled) {
        ...
        setSelectedImpasta(impastaTypes[0]); // Still set default impasta even if step is skipped
      } else if (categorySettings.bevande_enabled) {
        setCurrentStep('bevande');
        setSelectedImpasta(impastaTypes[0]); // Still set default impasta even if step is skipped
      } else {
        // If nothing is enabled, we'll handle this in the UI
        setCurrentStep('impasta');
        setSelectedImpasta(impastaTypes[0]);
      }
    }
  ...
  const handleNextStep = () => {
    if (currentStep === 'impasta' && selectedImpasta) {
      // Skip to next available step based on category settings
      if (categorySettings.aggiunti_enabled) {
        setCurrentStep('extras');
      } else if (categorySettings.bevande_enabled) {
        setCurrentStep('bevande');
      } else {
        // If neither extras nor bevande are enabled, go directly to cart
        handleAddToCart();
        return;
      }
    } else if (currentStep === 'extras') {
      // Skip to bevande if enabled, otherwise go to cart
      if (categorySettings.bevande_enabled) {
        setCurrentStep('bevande');
      } else {
        handleAddToCart();
  ...
  const handleBackStep = () => {
    if (currentStep === 'extras') {
      // Go back to impasta if enabled, otherwise this shouldn't happen
      if (categorySettings.impasto_enabled) {
        setCurrentStep('impasta');
      }
    } else if (currentStep === 'bevande') {
      // Go back to extras if enabled, otherwise go to impasta if enabled
      if (categorySettings.aggiunti_enabled) {
        setCurrentStep('extras');
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-6">
          ...
          {currentStep === 'bevande' && categorySettings.bevande_enabled && (
            <div>
              <div className="mb-4">
                <div className="flex items-center justify-between mb-2">
          ...
          {currentStep === 'bevande' && categorySettings.bevande_enabled && selectedBevande.length > 0 && (
            <>
              <Separator />
              <div>
          ...
          {((currentStep === 'bevande' && categorySettings.bevande_enabled) ||
            (currentStep === 'extras' && categorySettings.aggiunti_enabled && !categorySettings.bevande_enabled) ||
            (currentStep === 'impasta' && categorySettings.impasto_enabled && !categorySettings.aggiunti_enabled && !categorySettings.bevande_enabled)) && (
            <>
              <Separator />
              <div>
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-3">
              ...
              {currentStep === 'impasta' && categorySettings.impasto_enabled ? (
                ...
                <>
                  {(categorySettings.aggiunti_enabled || categorySettings.impasto_enabled) && (
                    <Button variant="outline" onClick={handleBackStep} className="flex-1">
                      Indietro
                    </Button>
                  )}
