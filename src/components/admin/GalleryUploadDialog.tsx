...
import { classifyUploadError, getUserFriendlyErrorMessage, logUploadError } from "@/utils/uploadErrorHandler";
  ...
  const handleUpload = useCallback(async (files: File[]) => {
    ...
    try {
      ...
      for (const file of files) {
        ...
        // Upload using unified upload service with automatic database synchronization
        const uploadResult = await uploadFileUnified(file, {
          uploadType: 'gallery',
          saveToDatabase: true,
          metadata: {
            title: file.name.split('.')[0] || "Immagine caricata",
            description: '',
            category: 'main',
            sort_order: 999,
            is_active: true,
        ...
        if (!uploadResult.success) {
          const classifiedError = classifyUploadError(uploadResult.error);
          const userMessage = getUserFriendlyErrorMessage(classifiedError);

          logUploadError(classifiedError, {
            fileName: file.name,
            fileSize: file.size,
            uploadType: 'gallery'
          });
          ...
          toast({
            ...
            description: `${file.name}: ${userMessage}`,
            variant: "destructive",
          });
          continue;
        }
        ...
        if (!uploadResult.url) {
          ...
          toast({
            title: "Caricamento fallito",
            description: `Nessun URL restituito per ${file.name}`,
            variant: "destructive",
          });
          continue;
        }
        ...
        // CRITICAL FIX: Save to database immediately after storage upload
        const dbResult = await saveImageToGalleryDatabase({
          id: newImage.id,
          title: newImage.alt,
          description: '',
          image_url: newImage.src,
          category: 'main',
          sort_order: 999, // Will be updated when images are reordered
          is_active: true,
          is_featured: newImage.featured
        });

        if (!dbResult.success) {
          console.error('❌ Failed to save image to database:', dbResult.error);
          toast({
            title: "Errore database",
            description: `Immagine caricata ma non salvata nel database: ${dbResult.error}`,
            variant: "destructive",
          });
          continue;
        }
