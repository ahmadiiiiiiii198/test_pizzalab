...
import { validateImageUrl, isBlobUrl } from "@/utils/urlUtils";
...
const GalleryImage = ({ src, alt, className = "", onLoad, onError }: GalleryImageProps) => {
  ...
  // Handle URL transformation using URL utilities
  const processImageUrl = useCallback((url: string) => {
    // Use consistent URL validation
    return validateImageUrl(url, fallbackImages[0]);
  }, []);
